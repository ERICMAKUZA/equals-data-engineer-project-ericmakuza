...
- Final Star Schema
This diagram illustrates the final analytics-ready model. The design is a classic Star Schema, which is the industry standard for data warehousing. It is optimized for high-performance analytical queries.

- Key Characteristics:

Denormalized: The fact_transactions table now has direct links (foreign keys) to all its related dimensions. This minimizes the number of JOIN operations required for a query, making them much faster.

Central Fact Table: The fact_transactions table is at the center, containing the core business metrics (amount) and foreign keys.

Descriptive Dimension Tables: The surrounding dim_ tables describe the "who, what, and when" of the transactions.

Table Descriptions
fact_transactions (Fact Table): Contains the individual transaction records. Each row is a unique event and links to the dimensions that describe it.

dim_customers (Dimension Table): A list of all unique customers and their attributes (name, email, etc.).

dim_accounts (Dimension Table): A list of all unique bank accounts and their attributes (type, balance, etc.).

dim_dates (Generated Dimension Table): A calendar table that was generated by the ETL process. It allows for easy time-based analysis (e.g., grouping by month or year) without needing to perform complex date functions in every query.

![Star Schema](images/startschemadiag.png)
...
- **🛡️ Cloud Trail**: Auditing & Logging
- **👤 Glu Catalog**: Schema validation and metadata tracking
...
### 🔴 Challenge 2: No Access to Clod 9 on Free Tier
**Problem**: An alternative tool is required to access the Document DB
```
**Resolution Method**: Create a jumpbox EC2 instance in the same VPC to access the Document D 
